...
const PayrollGridLayer = () => {
    ...
    useEffect(() => {
        const fetchPayrolls = async () => {
            ...
            try {
                ...
                const response = await axios.get(`${process.env.REACT_APP_API_URL}/api/payrolls`, {
                    headers: {
    ...
    const handleDelete = async (payrollId) => {
        ...
        try {
            const token = localStorage.getItem("token");
            await axios.delete(`${process.env.REACT_APP_API_URL}/api/payrolls/${payrollId}`, {
                headers: {
                    'Authorization': `Bearer ${token}`
                }
            });
    ...
    const displayedPayrolls = filteredPayrolls.slice(startIndex, startIndex + itemsPerPage);
            ...
            <div className="card-body p-24">
                <div className="row gy-4">
                    {displayedPayrolls.map((payroll) => (
                        <div key={payroll._id} className="col-xxl-3 col-md-6">
                            <div className="position-relative border radius-16 overflow-hidden">
                                <div className="bg-primary-50 p-16" style={{ height: '80px' }}>
                                    <div className="d-flex justify-content-between align-items-center h-100">
                                        <div>
                                            <h6 className="text-md mb-0">Salary</h6>
                                            <span className="text-primary-600 fw-medium">
                                                ${payroll.salary?.toLocaleString() || '0'}
                                            </span>
                                        </div>
                                ...
                                <div className="ps-16 pb-16 pe-16 text-center mt-16">
                                    <div className="d-flex justify-content-center">
                                        <div className="bg-primary-50 w-100-px h-100-px rounded-circle d-flex justify-content-center align-items-center">
                                            <Icon icon="mdi:cash" className="icon text-3xl text-primary-600" />
                                        </div>
                                    </div>
                                    <h6 className="text-lg mb-0 mt-12">{payroll.employee?.name || 'N/A'}</h6>
...
export default PayrollGridLayer;
